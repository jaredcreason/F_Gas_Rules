...
facilities_raw <- read_excel("Bryans code/TRI facility scripts/Final_list.xls") %>%
...
tri_facilities_raw <- read_csv("Bryans code/TRI facility scripts/tri_2019_us.csv") %>%
...
plot_usmap(include=c(.northeast_region,.south_region,.north_central_region), labels=TRUE, fill = "yellow", alpha = 0.05) +
  ggrepel::geom_label_repel(data = facilities_t,
                            aes(x = x , y = y , 
                                label = paste0(FACILITY.NAME,' - ', nearby_tri_1mi, " TRI facilities")),
                            size = 5, alpha = 0.8,
...
ggsave('TRI_facilities_1mi.png',width=11,height=8)

plot_usmap(include=c(.northeast_region,.south_region,.north_central_region), labels=TRUE, fill = "yellow", alpha = 0.05) +
  ggrepel::geom_label_repel(data = facilities_t,
                            aes(x = x , y = y, 
                                label = paste0(FACILITY.NAME,' - ', nearby_tri_3mi, " TRI facilities")),
                            size = 5, alpha = 0.8,
                            label.r = unit(0.5, "lines"), label.size = 0.5,
                            segment.color = "red", segment.size = 1,
                            seed = 1002) +
  ...
  labs(size = expression("Nearby TRI Facilities \n      (within 3 miles)")) +
  theme(legend.position = c(0.85, 0.1))
ggsave('TRI_facilities_3mi.png',width=11,height=8)

plot_usmap(include=c(.northeast_region,.south_region,.north_central_region), labels=TRUE, fill = "yellow", alpha = 0.05) +
  ggrepel::geom_label_repel(data = facilities_t,
                            aes(x = x , y = y, 
                                label = paste0(FACILITY.NAME,' - ', nearby_tri_5mi, " TRI facilities")),
                            size = 5, alpha = 0.8,
                            label.r = unit(0.5, "lines"), label.size = 0.5,
                            segment.color = "red", segment.size = 1,
                            seed = 1002) +
  ...
  labs(size = expression("Nearby TRI Facilities \n      (within 5 miles)")) +
  theme(legend.position = c(0.85, 0.1))
ggsave('TRI_facilities_5mi.png',width=11,height=8)

plot_usmap(include=c(.northeast_region,.south_region,.north_central_region), labels=TRUE, fill = "yellow", alpha = 0.05) +
  ggrepel::geom_label_repel(data = facilities_t,
                            aes(x = x , y = y, 
                                label = paste0(FACILITY.NAME,' - ', nearby_tri_10mi, " TRI facilities")),
                            size = 5, alpha = 0.8,
                            label.r = unit(0.5, "lines"), label.size = 0.5,
                            segment.color = "red", segment.size = 1,
                            seed = 1002) +
  ...
  labs(size = expression("Nearby TRI Facilities \n      (within 10 miles)")) +
  theme(legend.position = c(0.85, 0.1))
ggsave('TRI_facilities_10mi.png',width=11,height=8)
