...
## census_api_key("d8c412d907529ff6d86a8219eb08286fcaadf1a5", install=T,overwrite=T)
...
facilities <- read_excel('data\\facility_data\\Final_list.xls') %>%
...
urban_facilities <- st_intersection(facilities,uac)

facilities %<>% mutate(rural = fifelse(geometry %in% urban_facilities$geometry,0,1))
...
facilities_t <- usmap_transform(facilities_map) %>% mutate(FACILITY.NAME = str_to_title(FACILITY.NAME))

map <- plot_usmap(include=c(.northeast_region,.south_region,.north_central_region), labels=TRUE, fill = "yellow", alpha = 0.05) +
  ...
  geom_point(data = facilities_t,
             aes(x = LONGITUDE.1, y = LATITUDE.1, size = ghg_quantity),
...
# rural_tracts <- st_intersection(tr_pts,uac)
...
buffer = st_intersection(communities,shp) %>%
  select(GEOID,Tract,facility_id,"FACILITY.NAME")
...
facility_demographics_1mi <- merge(as.data.table(facilities), as.data.table(buffer),by="facility_id")
facility_demographics_3mi <- merge(as.data.table(facilities), as.data.table(buffer_3mi),by="facility_id")

facility_demographics_1mi <- merge(facility_demographics_1mi, table, by="GEOID") %>% 
              select('FACILITY NAME','CITY NAME',ghg_quantity,GEOID,sq_miles,rural.x,rural.y,pop,white,black,indian,asian,hispanic,income,pov50,pov99,total_risk,total_risk_resp) %>%
              rename(rural_facility = rural.x, rural_blockgroup = rural.y)

facility_demographics_3mi <- merge(facility_demographics_3mi, table, by="GEOID") %>% 
  select('FACILITY NAME','CITY NAME',ghg_quantity,GEOID,sq_miles,rural.x,rural.y,pop,white,black,indian,asian,hispanic,income,pov50,pov99,total_risk,total_risk_resp) %>%
  rename(rural_facility = rural.x, rural_blockgroup = rural.y)


facility_demographics_1mi %<>%
  group_by("'FACILITY NAME'","'CITY NAME'",ghg_quantity) %>%
  ...
  select("FACILITY NAME","CITY NAME",ghg_quantity,blockgroups_n,sq_miles,pop,pop_sq_mile_1mi,rural_facility,rural_bg_pct,white,black,indian,asian,hispanic,income,pov50,pov99,total_risk,total_risk_resp) %>% distinct()

  write.xlsx(facility_demographics_1mi,"data\\facility_data\\facility_demographics_1mi.xlsx")

facility_demographics_3mi %<>%
  group_by("'FACILITY NAME'","'CITY NAME'",ghg_quantity) %>%
  mutate(blockgroups_n = n(), sq_miles = sum(sq_miles, na.rm=TRUE), pop = sum(pop, na.rm=TRUE),white = sum(white, na.rm=TRUE),black = sum(black, na.rm=TRUE),indian = sum(indian, na.rm=TRUE),asian = sum(asian, na.rm=TRUE),hispanic = sum(hispanic, na.rm=TRUE),income = mean(income, na.rm=TRUE),pov50 = mean(pov50, na.rm=TRUE), pov99 = mean(pov99, na.rm=TRUE), total_risk = mean(total_risk, na.rm=TRUE), total_risk_resp = mean(total_risk_resp, na.rm=TRUE)) %>%
  mutate(pop_sq_mile_3mi = pop/sq_miles,
         rural_bg_pct = signif(sum(rural_blockgroup/blockgroups_n, na.rm=TRUE),2)) %>% ungroup() %>%
  select("FACILITY NAME","CITY NAME",ghg_quantity,blockgroups_n,sq_miles,pop,pop_sq_mile_3mi,rural_facility,rural_bg_pct,white,black,indian,asian,hispanic,income,pov50,pov99,total_risk,total_risk_resp) %>% distinct()
...
facilities_rural <- facilities[facilities$`FACILITY NAME`!="CHEMOURS LOUISVILLE WORKS" & facilities$`FACILITY NAME`!="DAIKIN AMERICA INC.",]

for (i in 1:length(facilities_rural)){
  
facility <- paste0(facilities_rural[i,]$`FACILITY NAME`)
...
write.xlsx(list_of_datasets,paste0("summary_tables_",facility,".xlsx"))
...
facilities_urban <- facilities[facilities$`FACILITY NAME`=="CHEMOURS LOUISVILLE WORKS" | facilities$`FACILITY NAME`=="DAIKIN AMERICA INC.",]

for (i in 1:length(facilities_urban)){
  
  facility <- paste0(facilities_urban[i,]$`FACILITY NAME`)
...
facilities_map %<>% mutate(total_risk_state_rural = c(28,36,30,52,33,52,28,30,42),
...
facilities_map %<>% mutate(total_risk_diff = signif(total_risk_1mi/total_risk_state_rural,2),
...
map <- plot_usmap(include=c(.northeast_region,.south_region,.north_central_region), labels=TRUE, fill = "yellow", alpha = 0.05) +
  ggrepel::geom_label_repel(data = facilities_t,
                            aes(x = LONGITUDE.1 , y = LATITUDE.1 , 
                                label = paste0(FACILITY.NAME,' - ', total_risk_diff )),
                            size = 5, alpha = 0.8,
                            label.r = unit(0.5, "lines"), label.size = 0.5,
                            segment.color = "red", segment.size = 1,
                            seed = 1002) +
...
map_resp <- plot_usmap(include=c(.northeast_region,.south_region,.north_central_region), labels=TRUE, fill = "yellow", alpha = 0.05) +
  ggrepel::geom_label_repel(data = facilities_t,
                            aes(x = LONGITUDE.1 , y = LATITUDE.1 , 
                                label = paste0(FACILITY.NAME,' - ', total_risk_diff_resp)),
                            size = 5, alpha = 0.8,
                            label.r = unit(0.5, "lines"), label.size = 0.5,
                            segment.color = "red", segment.size = 1,
                            seed = 1002) +
